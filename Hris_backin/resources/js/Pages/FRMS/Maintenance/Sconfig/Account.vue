...
const action = ref('')
const drawerTitle = computed(() => action.value === 'Add' ? 'Add Account' : action.value === 'Edit' ? 'Edit Account' : 'Account')

const headers = [
  { label: 'Account Code', fieldName: 'identitycode', type: 'link'  },
...
const loadData = async () => {
  ...
  const resp = await axios.get('/frls/transaction-code/frls-account-code')
...
const openLink = async (row) => {
  ...
  try {
    ...
    form.identitycode = data.identitycode ?? row.identitycode ?? ''
...
<template>
  <CardBox class="flex-1 p-6" has-table>
    <CoreTable
      ...
      table-name="FRMS Account Codes"
      searchable-fields="identitycode,description"
       :is-paginated="true"
      @openLink="openLink"
    >
