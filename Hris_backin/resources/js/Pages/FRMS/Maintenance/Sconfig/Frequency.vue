...
const action = ref('')
const drawerTitle = computed(() => action.value === 'Add' ? 'Add Frequency' : action.value === 'Edit' ? 'Edit Frequency' : 'Frequency')

const headers = [
 { label: 'Frequency Code', fieldName: 'identitycode',  type: 'link' },
...
const loadData = async () => {
  ...
  const resp = await axios.get('/frls/transaction-code/frls-frequincy')
...
<template>
  ...
  <AsideDrawer :title="drawerTitle" :is-open="openDrawer" @closeDrawer="openDrawer = false" class="shadow-lg shadow-blue-500/50">
    ...
    <FormField label="Frequency Code">
